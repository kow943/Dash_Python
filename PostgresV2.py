...
#Show Table selected and ask for what Chart you want
def getGraph(v):
    if v == None:
        ...
        return html.Div([
            ...
            # options=[{'label':x, 'value':x} for x in df.columns]),
            # html.P("Filter by:"),
            # dcc.Dropdown(id='filter-data',
            # options=[{'label':x, 'value':x} for x in df.columns]),
            # html.Button(id="submit-button", children="Create Graph", style = {'marginTop':40})
        ])
#--------------------------------------------------------------------------

#Get into different charts have different axis
def getGraphInfo(g, record):
    df = pd.DataFrame(record)
    if g == None:
        return dash.no_update
    else:
        if (g == 'Bar' or g == 'Line' or g == 'Scatter'):
            return html.Div([
                html.P("Insert X axis data"),
                dcc.Dropdown(id='xaxis-data',
                        options=[{'label':x, 'value':x} for x in df.columns]),
...
#To choose Axis for different charts
@app.callback(
    Output('chooseAxis', 'children'),
    Input('graph-data', 'value'),
    State('stored-data','data'),
)

def graphAxis(graphType, data):
    children = getGraphInfo(graphType, data)
    return children
#-----------------------------------------------------------------------------
...
def make_graphs(n, data, x_data, y_data, filter, chart_type):
    if n is None:
        return dash.no_update
    else:
        if chart_type == 'Histogram':
            bar_fig = px.histogram(data, x=x_data, y=y_data, color=filter, barmode='group')
